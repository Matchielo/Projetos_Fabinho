...
/* Pins Defitinioins */
// Output Pins
#define					Cut_MIC        GPIO_WriteHigh(GPIOB, GPIO_PIN_4)	 						  //Cut MIC
#define					Release_MIC    GPIO_WriteLow(GPIOB, GPIO_PIN_4) 								//Release MIC

#define 				Mut_AF         GPIO_WriteHigh(GPIOC, GPIO_PIN_6)  							//Bypass LM386
#define 				UnMut_AF       GPIO_WriteLow(GPIOC, GPIO_PIN_6)  								//UnBypass LM386

#define 				On_LineAF      GPIO_WriteHigh(GPIOC, GPIO_PIN_7)  							//Block Line>>AF
#define 				Off_LineAF     GPIO_WriteLow(GPIOC, GPIO_PIN_7)  								//UnBlock Line>>AF

#define 				Set_Lock       GPIO_WriteHigh(GPIOC, GPIO_PIN_5)  							//Turns On Lock
#define					Res_Lock       GPIO_WriteLow(GPIOC, GPIO_PIN_5)  								//Turns Off Lock

// Input Pins
#define 				Read_Call 			GPIO_ReadInputPin(GPIOD, GPIO_PIN_4)						//Read status Call button
#define 				Read_JPar 			GPIO_ReadInputPin(GPIOD, GPIO_PIN_5)						//Read status Parallel jumper
#define 				Read_JBot 			GPIO_ReadInputPin(GPIOD, GPIO_PIN_6)						//Read status Button jumper
	...
	while(1)
	{
		...
		if(AD_VLine < 40)
		{
			...
			if(Deb1_LineAF > 150)																																							// If VLine is low by more than 150 times, the phone is On Hook
			{
				...
				Off_LineAF;
				Cut_MIC;																																											// Cut the audio from Line to the AF output
			}
			...
			{
				Time++;
				PWM_Set_LineAF(450);		// 457 -> 3.473kHz
				Delay(40);
				TIM1_CR1 = 0;
				PWM_Set_LineAF(950);		// 921 -> 1.724kHz 
				Delay(40);
				TIM1_CR1 = 0;
				if(Read_Call)
				{
		...
		{
			                           //Cut_MIC;  OBS.: comentado o corte do MIC para manter impedancia baixa na linha e estabilizar mais rapido as tensoes no apertar das teclas KEY e AUX
			//Delay(12);
			VRef = AD_VLine;
			Off_LineAF;
			Delay(2);
...
void InitGPIO(void)
{
	...
	GPIO_Init(GPIOB, GPIO_PIN_4, GPIO_MODE_OUT_PP_LOW_FAST);														//Pin 12 -> MIC cutter [Needed to put a Pull Up external resistor 8k Ohm]
	GPIO_Init(GPIOC, GPIO_PIN_6 | GPIO_PIN_7, GPIO_MODE_OUT_PP_LOW_FAST);								//Pins 16 and 17 -> ([16]LM386 Bypass, [17] Line >> AF Blocker)
	GPIO_Init(GPIOC, GPIO_PIN_5, GPIO_MODE_OUT_PP_LOW_FAST);														//Pin 15 (Pulsed Lock Output) [Needed to put a Pull Up external resistor 8k Ohm]
}
...
void PWM_Set_LineAF(uint16_t Pwm_BzLine)
{
	...
	TIM1_CCR4H = (Set_PWM_Line >> 8);
	TIM1_CCR4L = Set_PWM_Line;
...
void PWM_Set_Alarm(uint16_t Pwm_Alarm)
{
	...
	TIM2_CCR3H = (Set_PWM_Alarm >> 8);
	TIM2_CCR3L = Set_PWM_Alarm;
	TIM2_CNTRH = 0;
	TIM2_CNTRL = 0;
	TIM2_CR1 |= 0b00000001;
}

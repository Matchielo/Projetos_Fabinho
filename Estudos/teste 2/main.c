 ...
 *
 * Versão atual: Contagem com interrupção de botão.
 */
...
#include "stm8s_exti.h"      // Biblioteca para Interrupções Externas (EXTI)

// ---------------------- Variáveis Globais (Voláteis) --------------------------
// Usadas para comunicação entre as ISRs e o loop principal
// **Estas foram movidas para o topo do arquivo para visibilidade global**
volatile uint8_t contagem = 0;      // 0=nenhuma, 1=14s, 2=24s
volatile uint8_t interrupcao = 0;   // Flag para sinalizar que uma interrupção ocorreu e possivelmente interromper a contagem
...
void main(void)
{
    // Variáveis de estado do botão para polling foram removidas, pois agora usamos interrupções.
    ...
    while (1)
    {
            // Verifica se uma contagem foi requisitada por uma interrupção
            if(contagem != 0)
            {
                // Resetamos o flag de interrupção ativa aqui, antes de iniciar a contagem.
                // Isso permite que uma nova interrupção (novo botão) seja detectada durante a contagem.
                interrupcao = 0;
                ...
                // Ao final da contagem (se não foi interrompida por outro botão), aciona o feedback
                // ou se foi interrompida, o feedback pode ser suprimido dependendo da sua lógica.
                // Aqui, o feedback é dado apenas se a contagem terminou naturalmente.
                if (interrupcao == 0) // Se a contagem não foi interrompida por outro botão
                {
                    BUZZER(1, 500); // Aciona o buzzer/LED uma vez por 500ms ao final da contagem
                }
                
                contagem = 0; // Limpa o pedido de contagem após a execução
            }
            // O microcontrolador pode ir para um modo de baixo consumo aqui (ex: WFI - Wait For Interrupt)
            // se não houver mais nada para fazer. Por simplicidade, vamos deixar o loop rodando.
    }
}

// ---------------------- Rotinas de Serviço de Interrupção (ISRs) --------------------------
// As interrupções externas para os pinos PD2 (BOT_1) e PD3 (BOT_2) do STM8.
// As ISRs são marcadas com @far @interrupt.

// ISR para o Botão 1 (conectado a PD2 - EXTI2)
@far @interrupt void EXTI2_IRQHandler(void)
{
    ...
    // Limpa o flag de interrupção diretamente no registrador EXTI_SR1
    // Acessa o registro EXTI_SR1 (Endereço: EXTI_BASE + 0x02) e escreve 1 no bit do pino 2 (0x04)
    // O Cosmic acessa os registros de periféricos diretamente pelo nome da macro.
    EXTI_SR1 = (uint8_t)GPIO_PIN_2; 
    
    enableInterrupts();
}

// ISR para o Botão 2 (conectado a PD3 - EXTI3)
@far @interrupt void EXTI3_IRQHandler(void)
{
    ...
    // Limpa o flag de interrupção diretamente no registrador EXTI_SR1
    // Acessa o registro EXTI_SR1 (Endereço: EXTI_BASE + 0x02) e escreve 1 no bit do pino 3 (0x08)
    EXTI_SR1 = (uint8_t)GPIO_PIN_3;
...
void Contagem_14s(void)
{
    ...
    for(i = 14; i >= 0; i--)
    {
        if (interrupcao) return; // Sai imediatamente se outro botão foi pressionado
...
void InitGPIO(void)
{
    ...
    // Pinos dos Botões: configurados como Entrada com Pull-up interno, COM interrupção.
    ...
    GPIO_Init(BOT_1_PORT, BOT_1_PIN, GPIO_MODE_IN_PU_IT);
    GPIO_Init(BOT_2_PORT, BOT_2_PIN, GPIO_MODE_IN_PU_IT);
    
    // EXTI_SENSITIVITY_FALL_ONLY significa que a interrupção será disparada
    // apenas quando o pino muda de HIGH para LOW (o que acontece quando o botão com pull-up é pressionado).

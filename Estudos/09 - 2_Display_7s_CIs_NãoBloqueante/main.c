...
void InitAWU_RTC(void);
void AWU_IRQHandler(void);

// ------------------- Variáveis Globais ------------------- //
volatile int g_contador_segundos = -1;			// Significa que nehuma contagem está ativa
volatile uint8_t g_contagem_ativa = 0;			// 0: Inativa / 1: Ativa
volatile uint8_t g_flags_1s = 0; 						// Flag Set by AWU Interrompe a cada 1s

// ------------------- Variaveis para detectar borda dos botões ------------------
...
uint8_t last_state_btn1;
uint8_t last_state_btn2;
...
void main(void)
{
	...
	InitAWU_RTC();	// Inicializa o RTC/AWU
	...
	while (1)
    {
        ...
        if ((last_state_btn1 == 1) && (current_state_btn1 == 0))
        {
            ...
            {
							g_contador_segundos = 14;			// Inicia a contagem
							g_contagem_ativa = 1;
							
							// Envia o dígito das unidades para os pinos BCD
							writeBCD(g_contador_segundos % 10); 		 // Calcula o dígito das unidades (resto da divisão por 10)
							pulseLatch(LATCH_01_PORT, LATCH_01_PIN); // Trava o valor no display das unidades
							
							// Envia o dígito das dezenas para os pinos BCD
							writeBCD(g_contador_segundos / 10);  			// Calcula o dígito das dezenas (resultado da divisão inteira por 10)
        ...
        if ((last_state_btn2 == 1) && (current_state_btn2 == 0))
        {
            ...
            if (ReadButton(BOT_2_PORT, BOT_2_PIN) == 0) // Confirma se continua pressionado
            {
				...
				// ---- Lógica de contagem não bloquenate -----
				// se a contagem está ativa e 1 segundo se passou
				if(g_contagem_ativa == 1 && g_flags_1s == 1) 
				{
					g_flags_1s = 0; // Limpa o Flag de 1 segundo
					
					if(g_contador_segundos > 0)
					{
						g_contador_segundos --; 			// Descrementa a contagem
						writeBCD(g_contador_segundos % 10); 
						pulseLatch(LATCH_01_PORT, LATCH_01_PIN);
						
						writeBCD(g_contador_segundos / 10);
						pulseLatch(LATCH_02_PORT, LATCH_02_PIN);
					}
					else // Contagem chegou a zero
					{
						g_contagem_ativa = 0; 			// Finaliza a contagem
						BUZZER(1, 300);
					}
				}
...
void InitCLOCK(void)
{
	...
	CLK_LSICmd(ENABLE); // Desabilita o oscilador interno de baixa velocidade (LSI), usado para RTC/AWU.
	...
	CLK_PeripheralClockConfig(CLK_PERIPHERAL_AWU, ENABLE); // Habilita o clock para AWU (Auto Wake-up Unit), que gerencia o RTC para alarmes/wake-up
...
void InitAWU_RTC(void)
{
	AWU_DeInit();			// Reseta as configurações do AWU
	
	// Configura o Prescaler do AWU. O LSI é cerca de 128kmh
	// Para ter 1s, precisamos que o timer do AWU conte (AWU_Timebase + 1) vezes.
	// LSI_Freq / (AWU_Prescaler_x * (AWU_Timebase + 1)) = 1Hz
  // Ex: LSI_Freq ~ 128kHz. Se AWU_Prescaler_32768, LSI_Freq/32768 = ~3.9Hz (freq do contador)
  // Para 1s, precisaríamos de (3.9 * X) = 1s, X = 0.25... difícil com int.
  // É mais fácil usar a base de tempo do AWU.

  // A função AWU_Init configura o prescaler e a base de tempo para atingir o período desejado.
  // Ex: AWU_Init(AWU_TIMEBASE_1S); configura o AWU para gerar uma interrupção a cada 1 segundo.
  // (Verifique a documentação do STM8S SPL para o valor exato do LSI e como ele se comporta com AWU_TIMEBASE).
  // Geralmente, AWU_TIMEBASE_1S é a opção mais direta.
	
	AWU_Init(AWU_TIMEBASE_1S);			// Configura o AWU para interrupçaõa de 1s
	
	AWU_ITConfig(ENABLE);			// Habilita a interrupção do AWU
	
	AWU_Cmd(ENABLE);			// Habilita o periféricoAWU
	
	enableInterrupts(); 			// Habilita interrupções Globais
}
...
void AWU_IRQHandler(void)
{
	if (AWU_GetFlagStatus() != RESET)
	{
		g_flags_1s = 1;			// Seta o Flag para indicar que 1s passou
		AWU_ClearFlag();		// Limpa o Flag de Interrupção
	}
}
